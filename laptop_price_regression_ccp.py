...
laptop_data = pd.read_csv(io.StringIO(uploaded[file_name].decode('utf-8')))
...
if target_column not in laptop_data.columns:
    raise KeyError(f"'{target_column}' not found in dataset columns")
X = laptop_data.drop(target_column, axis=1)
y = laptop_data[target_column]
...
significance_levels = np.linspace(0.1, 0.9, 9)
...
for alpha in significance_levels:
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X_transformed)):
        ...
        print(quant, len(nonconformity_scores))
        ...
        for i in range(len(y_pred_test)):
            ...
            lower_bounds = y_pred_test[i] - k
            upper_bounds = y_pred_test[i] + k
...
plt.plot(significance_levels, all_error_rates, marker='o', linestyle='-', color='b')
